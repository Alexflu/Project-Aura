            ...
            async def run():
                ...
                async with stdio_client(params) as (read, write):
                    async with ClientSession(read, write) as client:
                        ...
                        self.assertEqual(names, {"aura_status", "aura_propose_appearance", "aura_request_notepad", "aura_request_status"})
                        ...
                        result = await client.call_tool("aura_propose_appearance", {"hair": "bob", "palette": "ocean"})
                        self.assertFalse(result.is_error)
                        data = json.loads(result.content[0].text)
                        self.assertEqual(store.read()["look"]["hair"], "long")
